...
from src.data_pipeline.preprocess_audio import *
...
audio_dir_path = 'src/audio_pipeline/audio_samples_with_metadata/audio'
preprocessor_audio = AudioPreprocessor(audio_dir_path)
processed_audio = preprocessor_audio.preprocess()

# Create dataset and dataloader
dataset = BatchedDataset(processed_audio)
...
for batch in dataloader:
    batch = batch[0]  # Remove extra batch dimension introduced by DataLoader
    print("Batch shape:", batch.shape)  # Example: torch.Size([4, 85248])
    # Pass the batch to your model
    output = wav2vec_model(batch)
